...
class Email
{
	...
	{
		$req = $data['req'];
		$search = array('%name%', '%channame%', '%channameURL%', '%token%', '%tokenURL%', '%msg%');
		$replace = array($req->name, $req->cname, rawurlencode($req->cname), $data['token'], rawurlencode($data['token']), $req->reason);
